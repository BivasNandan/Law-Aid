...
const SearchSectionLaw = () => {
  ...
  // Handle delete law
  const canManage = userData && (userData.role === 'lawyer' || userData.role === 'admin')

  const handleDelete = async (lawId) => {
    if (!canManage) {
      return toast.error('Only lawyers can delete laws')
    }
  ...
  const handleAddLaw = () => {
    if (!canManage) {
      return toast.error('Please login as a lawyer to add laws')
    }
    navigate('/law-form')
  }
                ...
                <div className='grid grid-cols-1 md:grid-cols-5 gap-4 items-end'>
                  ...
                  <div className='md:col-span-1 flex items-end'>
                    <button 
                      ...
                      className={`w-full ${canManage ? 'bg-browntextcolor hover:bg-brownforhover text-white' : 'bg-gray-300 text-gray-600 cursor-not-allowed'} font-semibold py-3 px-6 rounded-lg transition-all duration-200 font-inria flex items-center justify-center gap-2`}
                      disabled={!canManage}
                    >
