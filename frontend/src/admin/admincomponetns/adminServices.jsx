            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-10 md:gap-12'>
                {services.map((service) => (
                  <div 
                    key={service.id}
                    onClick={() => {
                      // For admin-specific services ensure user is logged in, then navigate to the configured route
                      if ((service.id === 2 || service.id === 3) && (!userData || !userData._id)) {
                        return navigate('/login')
                      }
                      // For id 3 we previously appended the user id; admin routes are static
